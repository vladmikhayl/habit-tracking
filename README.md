...
| `gateway`             | `8080` | единая точка входа, сюда отправляются все запросы от пользователей или микросервисов. Здесь проверяется **JWT-токен** (для внешних запросов) или **сервисный токен** (для внутренних запросов), и если всё окей — запрос пересылается в нужный микросервис |
...
🔹 После поднятия всех приложений желательно подождать хотя бы 30 секунд, чтобы Eureka успела зарегистрировать все микросервисы
