...
Сначала нужно выполнить команду `docker-compose up -d` в корне проекта (это запустит **Postgres** и **Kafka**)

Далее нужно поочередно запустить приложения в порядке: `config-server`, `discovery`, `все остальные микросервисы`

### После запуска

✅ **Запросы можно отправлять на Gateway:** `http://localhost:8080/api/v1/...`
